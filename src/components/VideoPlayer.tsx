...
export const VideoPlayer: React.FC<VideoPlayerProps> = ({ 
  ...
  const handlePlayPause = () => {
    if (!isGoogleDrive && playerRef.current && isPlayerReady) {
      if (isPlaying) {
  ...
  const handleProgressChange = (newProgress: number) => {
    if (!isGoogleDrive && playerRef.current && isPlayerReady && duration > 0) {
  ...
  useEffect(() => {
    if (!isGoogleDrive && isPlayerReady && playerRef.current) {
      const updateTime = () => {
      ...
      // For Google Drive videos, set some default values since we can't control them
      setDuration(0);
      setCurrentTime(0);
      setProgress(0);
      setIsPlayerReady(true);
    }
  }, [isPlayerReady, isGoogleDrive]);
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        {isGoogleDrive ? (
          <div className="relative w-full h-full">
            ...
            <div className="absolute bottom-4 left-4 bg-black/70 text-white px-2 py-1 rounded text-sm">
              Google Drive Video - Use built-in controls
            </div>
          </div>
        ) : (
          <div 
      ...
      {/* Video Controls - Only show for non-Google Drive videos */}
      {!isGoogleDrive && (
      ...
      {/* Simplified controls for Google Drive videos */}
      {isGoogleDrive && (
        <div className="bg-white p-4">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600">
              Google Drive video - Controls available in the video player above
            </div>
            <div className="flex items-center gap-2">
              <button 
                onClick={onUnblur}
